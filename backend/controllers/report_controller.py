...
def delete_user(user_id, actor):
    ...
    try:
        ...
        refs = conn.execute('SELECT COUNT(*) AS c FROM transactions WHERE user_id=?', (user_id,)).fetchone()
        if (refs['c'] or 0) > 0:
            return 400, {'error': 'User has transaction history and cannot be deleted. Disable the account instead.'}
        ...
        conn.rollback()
        return 500, {'error': 'Could not delete user right now. Please try again.'}
    finally:
        conn.close()
